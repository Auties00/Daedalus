...
public final class ProtobufBinaryMemorySegmentReader extends ProtobufBinaryReader {
    ...
    @Override
    public ProtobufBinaryMemorySegmentReader readRawLengthDelimited(long size) {
        try {
            var result = new ProtobufBinaryMemorySegmentReader(segment.asSlice(position, size));
            position += size;
            return result;
        } catch (IndexOutOfBoundsException _) {
            throw ProtobufDeserializationException.truncatedMessage();
    ...
    public int[] readRawPackedVarInt32() {
        var length = readLengthDelimitedPropertyLength();
        var count = countVarInts(segment.asSlice(position, length).asByteBuffer());
        ...
        if (BMI2.isHardwareSupported()) {
            ...
            for (var i = 0; i < pairs; i++) {
                if (end - position < 16) break;
                var bv = ByteVector.fromMemorySegment(B128, segment, position, ByteOrder.LITTLE_ENDIAN);
