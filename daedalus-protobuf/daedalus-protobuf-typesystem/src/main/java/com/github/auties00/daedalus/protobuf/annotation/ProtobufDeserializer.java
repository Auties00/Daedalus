 ...
 * or in a type that should be interpreted as a {@link ProtobufMessage} or {@link ProtobufEnum}.
 *
 * <p> Implementing {@link ProtobufSize} is optional if the size of the serializer's return type can be calculated automatically,
 * but mandatory when writing directly to a {@link ProtobufBinaryWriter}, as the size cannot be inferred.
 *
 * <h2>Usage Example:</h2>
 ...
 * }
 * }</pre>
 * <h3>In a {@link ProtobufMessage} with delimited encoding:</h3>
 * <pre>{@code
 * @ProtobufMessage
 ...
 * }
 * }</pre>
 * <h3>With optional trailing parameters:</h3>
 * <pre>{@code
 * @ProtobufMessage
 * record EventPayload(byte[] data) {
 *     @ProtobufDeserializer
 *     static EventPayload fromBase64(String encoded, ProtobufOptimizationProfile profile) {
 *         return new EventPayload(Base64.getDecoder().decode(encoded));
 *     }
 * }
 * }</pre>
 * <h2>Low-Level Deserialization:</h2>
 * <p>For advanced use cases, you can deserialize directly from a {@link ProtobufBinaryReader} or {@link com.github.auties00.daedalus.protobuf.io.reader.ProtobufTextReader}.
 * <h3>From a Protobuf reader:</h3>
 * <pre>{@code
 * @ProtobufMessage
 * record BirthdayDate(int day, int month, int year) {
 *     @ProtobufDeserializer
 *     static BirthdayDate of(ProtobufBinaryReader reader) {
 ...
 *
 *     @ProtobufSerializer
 *     void writeToStream(ProtobufBinaryWriter writer) {
