...
import static com.github.auties00.daedalus.protobuf.io.reader.text.tokenized.ProtobufTextTokenKind.*;
...
public final class ProtobufTextTokenizedReader extends ProtobufTextReader {
    ...
    static {
        ...
        CHAR_CLASS[' ']  = CHAR_WHITESPACE;
        CHAR_CLASS['\t'] = CHAR_WHITESPACE;
        CHAR_CLASS['\r'] = CHAR_WHITESPACE;
        CHAR_CLASS['\n'] = (byte) (CHAR_WHITESPACE | CHAR_NEWLINE);

        CHAR_CLASS['{']  = CHAR_STRUCTURAL;
        CHAR_CLASS['}']  = CHAR_STRUCTURAL;
        CHAR_CLASS['<']  = CHAR_STRUCTURAL;
        CHAR_CLASS['>']  = CHAR_STRUCTURAL;
        CHAR_CLASS['[']  = CHAR_STRUCTURAL;
        ...
        CHAR_CLASS['"']  = CHAR_DQUOTE;
        CHAR_CLASS['\''] = CHAR_SQUOTE;
        CHAR_CLASS['#']  = CHAR_HASH;
        CHAR_CLASS['\\'] = CHAR_BACKSLASH;

        for (int c = '0'; c <= '9'; c++) {
            CHAR_CLASS[c] = CHAR_DIGIT_OR_DOT;
        }
        CHAR_CLASS['.'] = CHAR_DIGIT_OR_DOT;

        // Populate the structural-to-kind mapping
        STRUCTURAL_KIND['{'] = BRACE_OPEN;
        STRUCTURAL_KIND['}'] = BRACE_CLOSE;
        STRUCTURAL_KIND['<'] = ANGLE_OPEN;
        STRUCTURAL_KIND['>'] = ANGLE_CLOSE;
        STRUCTURAL_KIND['['] = BRACKET_OPEN;
        STRUCTURAL_KIND[']'] = BRACKET_CLOSE;
        STRUCTURAL_KIND[':'] = COLON;
        STRUCTURAL_KIND[','] = SEP;
        STRUCTURAL_KIND[';'] = SEP;
        STRUCTURAL_KIND['-'] = MINUS;

        // Pre-compute broadcast vectors
        V_SPACE    = ByteVector.broadcast(SPECIES, (byte) ' ');
        V_TAB      = ByteVector.broadcast(SPECIES, (byte) '\t');
        V_CR       = ByteVector.broadcast(SPECIES, (byte) '\r');
        V_LF       = ByteVector.broadcast(SPECIES, (byte) '\n');
        V_LBRACE   = ByteVector.broadcast(SPECIES, (byte) '{');
        V_RBRACE   = ByteVector.broadcast(SPECIES, (byte) '}');
        V_LANGLE   = ByteVector.broadcast(SPECIES, (byte) '<');
        V_RANGLE   = ByteVector.broadcast(SPECIES, (byte) '>');
        V_LBRACKET = ByteVector.broadcast(SPECIES, (byte) '[');
        V_RBRACKET = ByteVector.broadcast(SPECIES, (byte) ']');
        V_COLON    = ByteVector.broadcast(SPECIES, (byte) ':');
        V_COMMA    = ByteVector.broadcast(SPECIES, (byte) ',');
        V_SEMI     = ByteVector.broadcast(SPECIES, (byte) ';');
        V_MINUS    = ByteVector.broadcast(SPECIES, (byte) '-');
        V_DQUOTE   = ByteVector.broadcast(SPECIES, (byte) '"');
        V_SQUOTE   = ByteVector.broadcast(SPECIES, (byte) '\'');
        V_HASH     = ByteVector.broadcast(SPECIES, (byte) '#');
        V_BACKSLASH= ByteVector.broadcast(SPECIES, (byte) '\\');
    }
    ...
    public void readStartObject() {
        var kind = ProtobufTextToken.kind(next());
    ...
    public void readStartArray() {
        if (ProtobufTextToken.kind(next()) != BRACKET_OPEN) {
    ...
    public void readEndArray() {
        if (ProtobufTextToken.kind(next()) != BRACKET_CLOSE) {
    ...
    public boolean readBoolProperty() {
        var token = next();
        var kind = ProtobufTextToken.kind(token);
        boolean result;
        if (kind == IDENT) {
    ...
    public String readEnumNameProperty() {
        var token = next();
        if (ProtobufTextToken.kind(token) != IDENT) {
            throw new ProtobufDeserializationException("Expected enum name");
        }
        var result = tokenText(token);
        skipOptionalSeparator();
        return result;
    }
    ...
    private int peekKind() {
        return ProtobufTextToken.kind(tape.get(tapePosition));
    }

    private String tokenText(long token) {
        var offset = ProtobufTextToken.offset(token);
        var length = ProtobufTextToken.length(token);
    ...
    private long readRawInteger() {
        ...
        var token = next();
        if (ProtobufTextToken.kind(token) != NUMERIC) {
    ...
    private double readRawFloat() {
        ...
        if (kind == IDENT) {
            ...
            value = switch (text) {
                ...
                default -> throw new ProtobufDeserializationException(
                        "Unexpected identifier in numeric context: " + text);
            };
        } else if (kind == NUMERIC) {
            value = Double.parseDouble(tokenText(token));
        } else {
    ...
    private void unescapeStringInto(StringBuilder sb, long token) {
        var offset = ProtobufTextToken.offset(token) + 1;
        var length = ProtobufTextToken.length(token) - 2;
     ...
     * {@link ProtobufTextToken#pack(int, int, int)}.
     *
    ...
    private LongBuffer tokenize() {
        ...
        return LongBuffer.wrap(tapeArray, 0, count).asReadOnlyBuffer();
    }
    ...
    private int tokenizeSinglePass(long[] tape) {
        ...
        if (state.pendingContentStart >= 0) {
            ...
            tape[tapePos++] = ProtobufTextToken.pack(state.pendingContentStart, len, state.pendingContentKind);
        }
        ...
        if (state.pendingStringStart >= 0) {
            ...
            tape[tapePos++] = ProtobufTextToken.pack(state.pendingStringStart, len, state.pendingStringKind);
        }
    ...
    private int processMaskingAndExtraction(long whitespace, long structural,
        ...
        while (allStarts != 0) {
            ...
            if ((activeStructural & bit) != 0) {
                ...
                tape[tapePos++] = ProtobufTextToken.pack(absPos, 1, STRUCTURAL_KIND[b]);
                ...
                if (pos + runLen >= chunkLen) {
                    ...
                    tape[tapePos++] = ProtobufTextToken.pack(absPos, runLen, kind);
                }
            }
    ...
    private int emitString(int pos, long inStringMask, int chunkStart, int chunkLen,
        ...
        if (closePos < chunkLen) {
            ...
            tape[tapePos++] = ProtobufTextToken.pack(chunkStart + pos, length, kind);
        } else {
    ...
    private int handlePendingString(ParseState state, int chunkStart, int chunkLen,
        ...
        tape[tapePos++] = ProtobufTextToken.pack(state.pendingStringStart, length, state.pendingStringKind);
    ...
    private int handlePendingContent(ParseState state, int chunkStart,
        ...
        if ((content & 1) == 0) {
            ...
            tape[tapePos++] = ProtobufTextToken.pack(state.pendingContentStart, totalLength, state.pendingContentKind);
