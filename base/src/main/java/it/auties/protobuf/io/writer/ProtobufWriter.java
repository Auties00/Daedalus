...
import it.auties.protobuf.io.calculator.ProtobufSizeCalculator;
 ...
 * @see ProtobufReader
 */
public abstract class ProtobufWriter<OUTPUT> implements AutoCloseable {
    ...
    public static ProtobufWriter<byte[]> toBytes(int length) {
        if(length < 0) {
            ...
            return new ProtobufByteArrayWriter(new byte[length], 0);
        }
    }
    ...
    public static ProtobufWriter<byte[]> toBytes(byte[] bytes, int offset) {
        ...
        return new ProtobufByteArrayWriter(bytes, offset);
    }
    ...
    public static ProtobufWriter<ByteBuffer> toHeapBuffer(int length) {
        if(length < 0) {
            ...
            return new ProtobufByteBufferWriter(ByteBuffer.allocate(length));
        }
    }
    ...
    public static ProtobufWriter<ByteBuffer> toDirectBuffer(int length) {
        if(length < 0) {
            ...
            return new ProtobufByteBufferWriter(ByteBuffer.allocateDirect(length));
        }
    }
    ...
    public static ProtobufWriter<ByteBuffer> toBuffer(ByteBuffer buffer) {
        ...
        if(buffer.isReadOnly()) {
            ...
            return new ProtobufByteBufferWriter(buffer);
        }
    }

    public static ProtobufWriter<MemorySegment> toMemorySegment(MemorySegment segment) {
        Objects.requireNonNull(segment, "segment must not be null");
        return new ProtobufMemorySegmentWriter(segment);
    }

    public static ProtobufWriter<OutputStream> toStream(OutputStream buffer) {
        Objects.requireNonNull(buffer, "buffer must not be null");
        return new ProtobufStreamWriter(buffer);
    }  
    
    public static ProtobufWriter<OutputStream> toStream(OutputStream buffer, int tempBufferLength) {
        Objects.requireNonNull(buffer, "buffer must not be null");
        if(tempBufferLength < 0) {
            throw new IllegalArgumentException("tempBufferLength must not be negative");
        }
        return new ProtobufStreamWriter(buffer, tempBufferLength);
    }
    ...
    public void writePackedInt32Property(long propertyIndex, int[] values) {
        if(values != null){
            var size = 0;
            for(var value : values) {
                size += ProtobufSizeCalculator.getVarInt32Size(value);
            }
            writePropertyTag(propertyIndex, ProtobufWireType.WIRE_TYPE_LENGTH_DELIMITED);
            writeLengthDelimitedPropertyLength(size);
    ...
    public void writePackedInt64Property(long propertyIndex, long[] values) {
        if(values != null){
            var size = 0;
            for(var value : values) {
                size += ProtobufSizeCalculator.getVarInt64Size(value);
            }
            writePropertyTag(propertyIndex, ProtobufWireType.WIRE_TYPE_LENGTH_DELIMITED);
            writeLengthDelimitedPropertyLength(size);
