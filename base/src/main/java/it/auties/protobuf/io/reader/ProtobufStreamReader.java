...
class ProtobufStreamReader extends ProtobufReader {
    ...
    private ProtobufStreamReader lengthDelimitedReader;

    ProtobufStreamReader(InputStream inputStream, boolean autoclose, long limit) {
        this(inputStream, autoclose, limit, DEFAULT_BUFFER_SIZE);
    }

    ProtobufStreamReader(InputStream inputStream, boolean autoclose, long limit, int bufferSize) {
    ...
    private ProtobufStreamReader(InputStream inputStream, long limit, byte[] buffer, int bufferPosition, int bufferLimit, long position) {
        this.inputStream = inputStream;
    ...
    @Override
    public ProtobufStreamReader readRawLengthDelimited(long lengthDelimitedPropertyLength) {
        assertReadable();
        if (lengthDelimitedPropertyLength >= 0) {
            return lengthDelimitedReader = new ProtobufStreamReader(
